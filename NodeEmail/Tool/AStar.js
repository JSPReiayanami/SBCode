...
AStar.prototype.FindPath = function(pos,targetPos){
    ...
    while(curNode != null){
        ...
        if(curNode.x == this.m_EndPos.x && curNode.y == this.m_EndPos.y){
...
AStar.prototype.OpenNode = function(node){
    ...
    for(let index in findOp){
        ...
        let node = this.CreateNode({ x:pos.x+opObject.x,y:pos.y+opObject.y},{LastNode:node,G:node.m_G+1})
...
AStar.prototype.GetGoodOpenNode = function(node){
    
}

AStar.prototype.CreateNode = function(pos,data){
    if(this.m_Map[pos.x] == null || this.m_Map[pos.y] == null) return null
...
AStar.prototype.CanMove = function(pos){
    ...
    return this.m_CanMoveCall(pos,this.m_Map[pos.x][pos.y])
}
